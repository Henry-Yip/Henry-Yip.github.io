...
This is a post on Classical Mechanics in High School style. It serves as a good introduction to High Schoolers wanting to pursue a deeper understanding in Physics. 
...
$$ \omega$$ refers to angular velocity. We have $$ v= r \omega$$. 

$$\dot{r}$$ is the derivative of $$r$$ (with respect to time). 

We have $$F=m \ddot{r}$$ (Make sure you can do this!)

Here we denote $$\omega= \dot{\theta}$$, $$\dot{\theta}$$ is more often used as it shows dependence on $$\theta$$.
...
This phenonmoenon is quite surprising because not all paticles have the same orbits while revolving around a star, some have circular and some have really weird shape.s This theorem generalizes for all such orbits shapes however, given that the forces are conservative. 

This theorem is useful for caluclating "Jean's Mass", as I learnt it in my Year 1 introductory Astrophysics course.

There is indeed a really nice way to think about it, given by Dr Ross Galloway, one of the lecturers for Introductory Astrophysics:
...
Hence gravitational potential energy is twice the kinetic energy for this case. 
...
{% responsive_image path:"assets/Mathematica.png" %}

Hence we have:

$$\frac{-\pi R}{2}= -\sqrt\frac{{2G(m_1 + m_2)}}{R} T$$

Resolving it will get us:

$$
T= \frac{\pi}{2} \sqrt{\frac{R^3}{2G(m_1+m_2)}}
$$
